...
- `./mvnw test` → **`Tests run: 9, Failures: 0, Errors: 0` — BUILD SUCCESS**
  (`JwtServiceTest`, `StampTokenServiceTest` — controles de cripto não regrediram).
- `src/main` alterado em 12 arquivos + 2 de config; `src/test` adicionado.
...
| SEC-016 | FIX-16 | Política de senha de staff (`@Size(min=10,max=72)`), `@Email` no e-mail; BCrypt custo **12** | `CreateStaffUserRequest.java`, `BcryptConfig.java` | Inspeção: anotações presentes; encoder `new BCryptPasswordEncoder(12)`. Hashes antigos seguem válidos. | **Fechado (estático)** · dinâmico: TC-AUTH-04 |
| SEC-017 (formato) | FIX-17 | PIN validado por `@Pattern(\\d{4,10})` + `@Valid` no controller | `SetPinRequest.java`, `AdminController.java` | Inspeção: `@Pattern`/`@Valid` presentes. | **Fechado (estático)** · dinâmico: TC-AUTH-18 |
| SEC-017 (brute force) | FIX-04 | **Não aplicado** (rate limit/lockout exige Bucket4j) | — | — | **Aberto** (Parcial no conjunto) |
| SEC-022 | FIX-07 | `@Email`/`@Size` nos DTOs de cliente; `@Valid` em `loginOrRegister`; `@Min/@Positive` no programa | `CustomerLoginRequest`, `CreateCustomerRequest`, `CustomerController`, `CreateProgramRequest`, `UpdateProgramRequest` | Inspeção: anotações + `@Valid` presentes. **Ressalva:** retipar `StampRequest.payload` **não** foi feito (contrato). | **Parcial** · dinâmico: TC-INPUT-03/04 |
| SEC-035 | FIX-24 | `@Min(1)` em `ruleTotalStamps`, `@Positive`/`@PositiveOrZero` | `CreateProgramRequest`, `UpdateProgramRequest` | Inspeção: anotações presentes. | **Fechado (estático)** · dinâmico: TC-BIZ-10 |
