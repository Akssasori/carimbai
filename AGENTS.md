...
./mvnw spring-boot:run               # profile `dev` (via .Codex/settings.json)
```
...
- Veja regras detalhadas de código em `.Codex/rules/java.md`.

## MCP

`.mcp.json` define um servidor **Postgres** opcional para inspeção do banco em
dev. Ele usa `${CARIMBAI_DB_URL}` (defina em `.Codex/settings.local.json` →
